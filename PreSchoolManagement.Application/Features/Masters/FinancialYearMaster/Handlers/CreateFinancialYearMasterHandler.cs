...
public class CreateFinancialYearMasterHandler(IFinancialYearMasterService service, 
    IValidator<CreateFinancialYearMasterCommand> validator, IMapper mapper,ICurrentUserService currentUser) 
    : IRequestHandler<CreateFinancialYearMasterCommand, ApiResponse<int>>
{
    public async Task<ApiResponse<int>> Handle(CreateFinancialYearMasterCommand request, CancellationToken cancellationToken)
    {
        ...
        if (exists)
            return ApiResponse<int>.FailureResponse(MessageHelper.AlreadyExists(EntityDescription.FinancialYear.ToString()), (int)HttpStatusCode.Conflict);
        ...
        (
            ...
            MessageHelper.Added(EntityDescription.FinancialYear.ToString()), 
